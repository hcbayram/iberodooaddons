# -*- coding: utf-8 -*-
"""
Genel amaçlı UBL-TR gelen fatura XML ayrıştırıcısı.

Her entegratör (Uyumsoft, Hızlı, NES) kendi bağlantısı üzerinden aynı
mantıkla ("_parse_ubl_xml") XML ayrıştırıyordu — bu dosya, entegratöre
bağımlı olmayan (elle XML içe aktarma gibi) senaryolar için o mantığın
tekilleştirilmiş, en eksiksiz (Hızlı'nın iskonto/ek vergi/tevkifat
alanlarını da kapsayan) bir kopyasıdır.

Dönüş şekli, ``invoice_integrator_sync.py``'deki ``_map_incoming_invoice``/
``_create_invoice_lines``'ın beklediği "de facto sözleşme" ile birebir
uyumludur: ``{"header": {...}, "data": [...], "documentTotals": {...},
"xml_clean": xml_text}``.
"""
...
def _party_tax_number(party_el, ns=_NS):
    """VKN/TCKN'yi önce cac:PartyTaxScheme/cbc:CompanyID'den, orada yoksa
    cac:PartyIdentification[schemeID=VKN|TCKN]/cbc:ID'den okur — GİB
    UBL-TR XML'lerinde ikisi de kullanılabiliyor (canlı örnekte
    PNR2026000000115, 2026-08-13'te tespit edildi)."""
    if party_el is None:
        return ""
...
def parse_incoming_ubl_xml(xml_text: str) -> dict:
    """UBL-TR XML'ini _map_incoming_invoice/_create_invoice_lines'ın
    beklediği formata çevirir. GİB'in zorunlu kıldığı cbc:UUID de
    (entegratör API'lerinin aksine) burada ayrıca okunur — elle XML içe
    aktarımda kaydın UUID alanını doldurmak için tek kaynak budur."""
    root = ET.fromstring(xml_text.encode("utf-8") if isinstance(xml_text, str) else xml_text)
    header = {
        "id": _xt(root, "cbc:ID"),
        "uuid": _xt(root, "cbc:UUID"),
