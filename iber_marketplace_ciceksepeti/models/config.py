...
from ..core.ciceksepeti_const import CARGO_COMPANIES


class IberCiceksepetiConfig(models.Model):
    _name = 'iber.marketplace.ciceksepeti.config'
    _description = 'Çiçeksepeti Bağlantı Ayarları'
    _rec_name = 'name'
    ...
    order_ids = fields.One2many(
        'iber.marketplace.ciceksepeti.order', 'config_id', string='Siparişler')
    ...
    listing_ids = fields.One2many(
        'iber.marketplace.ciceksepeti.listing', 'config_id', string='Ürün Listesi')
    ...
    def _compute_counts(self):
        pending_statuses = ('Created', 'Preparing')
        for rec in self:
    ...
    def _sync_hub_channel(self):
        ...
        if not Channel:
            return
        for rec in self:
            Channel._sync_from_config('ciceksepeti', rec)
    ...
    def _get_client(self):
        self.ensure_one()
        from ..core.ciceksepeti_client import CiceksepetiClient
        return CiceksepetiClient(api_key=self.api_key)
    ...
    def action_test_connection(self):
        ...
        try:
            ...
            client.get_orders(page=0, page_size=1)
            self.write({'connection_state': 'connected', 'last_error': False})
            return {
                'type': 'ir.actions.client',
                'tag': 'display_notification',
                'params': {
                    'message': _('Çiçeksepeti bağlantısı başarılı.'),
                    'type': 'success',
                    'sticky': False,
                },
            }
    ...
    def action_view_orders(self):
        ...
        return {
            ...
            'name': _('Siparişler'),
            'res_model': 'iber.marketplace.ciceksepeti.order',
            'view_mode': 'list,form',
    ...
    def action_view_listings(self):
        ...
        return {
            'type': 'ir.actions.act_window',
            'name': _('Ürün Listesi'),
            'res_model': 'iber.marketplace.ciceksepeti.listing',
            'view_mode': 'list,form',
            'domain': [('config_id', '=', self.id)],
            'context': {'default_config_id': self.id},
        }
    ...
    def action_open_sync_wizard(self):
        ...
        return {
            ...
            'res_model': 'iber.marketplace.ciceksepeti.sync.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {
    ...
    def action_cron_sync_orders(self):
        ...
        for cfg in configs:
            try:
                wizard = self.env['iber.marketplace.ciceksepeti.sync.wizard'].create({
                    'config_id': cfg.id,
                    'sync_type': 'orders',
