...
from ..core.hepsiburada_const import CARGO_COMPANIES


class IberHepsiburadaConfig(models.Model):
    _name = 'iber.marketplace.hepsiburada.config'
    _description = 'Hepsiburada Bağlantı Ayarları'
    _rec_name = 'name'
    ...
    # Hepsiburada kimlik bilgileri
    username = fields.Char(required=True, string='Kullanıcı Adı')
    password = fields.Char(required=True, string='Şifre', password=True)
    merchant_id = fields.Char(
        string='Satıcı ID',
        help='Hepsiburada Merchant ID (ürün listesi için)')
    ...
    order_ids = fields.One2many(
        'iber.marketplace.hepsiburada.order', 'config_id', string='Siparişler')
    ...
    listing_ids = fields.One2many(
        'iber.marketplace.hepsiburada.listing', 'config_id', string='Ürün Listesi')
    ...
    def _compute_counts(self):
        pending_statuses = ('New', 'Processing', 'Shipping')
        for rec in self:
    ...
    def _sync_hub_channel(self):
        ...
        if not Channel:
            return
        for rec in self:
            Channel._sync_from_config('hepsiburada', rec)
    ...
    def _get_client(self):
        self.ensure_one()
        from ..core.hepsiburada_client import HepsiburadaClient
        return HepsiburadaClient(
            username=self.username,
            password=self.password,
            merchant_id=self.merchant_id or None,
        )
    ...
    def action_test_connection(self):
        ...
        try:
            ...
            client.get_orders(offset=0, limit=1)
            self.write({'connection_state': 'connected', 'last_error': False})
            return {
                'type': 'ir.actions.client',
                'tag': 'display_notification',
                'params': {
                    'message': _('Hepsiburada bağlantısı başarılı.'),
                    'type': 'success',
                    'sticky': False,
                },
            }
    ...
    def action_view_orders(self):
        ...
        return {
            ...
            'name': _('Siparişler'),
            'res_model': 'iber.marketplace.hepsiburada.order',
            'view_mode': 'list,form',
    ...
    def action_view_listings(self):
        ...
        return {
            'type': 'ir.actions.act_window',
            'name': _('Ürün Listesi'),
            'res_model': 'iber.marketplace.hepsiburada.listing',
            'view_mode': 'list,form',
            'domain': [('config_id', '=', self.id)],
            'context': {'default_config_id': self.id},
        }
    ...
    def action_open_sync_wizard(self):
        ...
        return {
            ...
            'res_model': 'iber.marketplace.hepsiburada.sync.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {
    ...
    def action_cron_sync_orders(self):
        ...
        for cfg in configs:
            try:
                wizard = self.env['iber.marketplace.hepsiburada.sync.wizard'].create({
                    'config_id': cfg.id,
                    'sync_type': 'orders',
