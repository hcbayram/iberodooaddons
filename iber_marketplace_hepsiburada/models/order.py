...
class IberHepsiburadaOrder(models.Model):
    _name = 'iber.marketplace.hepsiburada.order'
    ...
    # Hepsiburada alanları
    hb_order_id = fields.Char(
        index=True, readonly=True, string='HB Sipariş ID')
    ...
    # Kargo bilgileri
    cargo_company = fields.Char(readonly=True, string='Kargo Şirketi (HB)')
    ...
    line_ids = fields.One2many(
        'iber.marketplace.hepsiburada.order.line', 'order_id', string='Kalemler')
    ...
    def action_send_shipment(self):
        ...
        for line in self.line_ids:
            cargo = (
                line.cargo_company
                or self.config_id.cargo_company
                or ''
            )
            if not cargo:
                raise UserError(
            ...
            try:
                client.send_shipment(
                    order_id=self.hb_order_id,
                    line_item_id=line.hb_line_id,
                    cargo_company=cargo,
        ...
        if errors:
            raise UserError(_('Kargo bilgisi gönderilemedi:\n%s') % '\n'.join(errors))
        ...
        return {
            ...
            'params': {
                'message': _("Kargo bilgisi başarıyla Hepsiburada'ya gönderildi."),
                'type': 'success',
                'sticky': False,
            },
        }
    ...
    def action_create_sale_order(self):
        ...
        if not partner:
            partner = self.env['res.partner'].create({
                'name': self.customer_name or _('Hepsiburada Müşterisi'),
        ...
        sale_vals = {
            ...
            'company_id': self.company_id.id,
            'origin': self.order_number or f'Hepsiburada #{self.hb_order_id}',
            'order_line': [],
        }
        for line in self.line_ids:
            ...
            if not product:
                product = self.env['product.product'].search(
                    [('default_code', '=', line.merchant_sku)], limit=1
                )
            if not product:
                continue
