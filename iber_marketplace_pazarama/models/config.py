...
from ..core.pazarama_const import CARGO_COMPANIES


class IberPazaramaConfig(models.Model):
    _name = 'iber.marketplace.pazarama.config'
    _description = 'Pazarama Bağlantı Ayarları'
    _rec_name = 'name'
    ...
    # Pazarama kimlik bilgileri
    client_id = fields.Char(required=True, string='Client ID')
    client_secret = fields.Char(required=True, password=True, string='Client Secret')

    # Önbelleğe alınmış token bilgisi
    access_token = fields.Char(readonly=True, copy=False, string='Access Token')
    token_expiry = fields.Datetime(readonly=True, copy=False, string='Token Bitiş')
    ...
    order_ids = fields.One2many(
        'iber.marketplace.pazarama.order', 'config_id', string='Siparişler')
    ...
    listing_ids = fields.One2many(
        'iber.marketplace.pazarama.listing', 'config_id', string='Ürün Listesi')
    ...
    def _compute_counts(self):
        pending_statuses = ('Created', 'Approved', 'Preparing', 'ReadyForShipment')
        for rec in self:
    ...
    def _sync_hub_channel(self):
        ...
        if not Channel:
            return
        for rec in self:
            Channel._sync_from_config('pazarama', rec)
    ...
    def _get_client(self):
        self.ensure_one()
        from ..core.pazarama_client import PazaramaClient

        def _on_token_refresh(token, expiry):
            self.sudo().write({
                'access_token': token,
                'token_expiry': expiry.strftime('%Y-%m-%d %H:%M:%S'),
            })

        return PazaramaClient(
            client_id=self.client_id,
            client_secret=self.client_secret,
            access_token=self.access_token or None,
            token_expiry=(
                self.token_expiry.replace(tzinfo=None) if self.token_expiry else None
            ),
    ...
    def action_test_connection(self):
        ...
        try:
            ...
            return {
                ...
                'params': {
                    'message': _('Pazarama bağlantısı başarılı.'),
                    'type': 'success',
                    'sticky': False,
                },
            }
    ...
    def action_view_orders(self):
        ...
        return {
            ...
            'name': _('Siparişler'),
            'res_model': 'iber.marketplace.pazarama.order',
            'view_mode': 'list,form',
    ...
    def action_view_listings(self):
        ...
        return {
            'type': 'ir.actions.act_window',
            'name': _('Ürün Listesi'),
            'res_model': 'iber.marketplace.pazarama.listing',
            'view_mode': 'list,form',
            'domain': [('config_id', '=', self.id)],
            'context': {'default_config_id': self.id},
        }
    ...
    def action_open_sync_wizard(self):
        ...
        return {
            ...
            'res_model': 'iber.marketplace.pazarama.sync.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {
    ...
    def action_cron_sync_orders(self):
        ...
        for cfg in configs:
            try:
                wizard = self.env['iber.marketplace.pazarama.sync.wizard'].create({
                    'config_id': cfg.id,
                    'sync_type': 'orders',
