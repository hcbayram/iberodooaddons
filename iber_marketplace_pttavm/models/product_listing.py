...
class IberPttavmListing(models.Model):
    _name = 'iber.marketplace.pttavm.listing'
    _description = 'PttAVM Ürün Listesi'
    _rec_name = 'barcode'
    ...
    barcode = fields.Char(required=True, index=True, string='Barkod')
    active = fields.Boolean(default=True)

    # PttAVM'den gelen bilgiler
    pttavm_stock = fields.Integer(readonly=True, string='PttAVM Stoku')
    pttavm_price = fields.Float(readonly=True, string='PttAVM Fiyatı')
    ...
    def action_push_inventory(self):
        """Seçili listingları PttAVM'e stok ve fiyat olarak gönder."""
        ...
        for cfg in configs:
            ...
            for listing in group_listings:
                stock_items.append({
                    ...
                    'stockQuantity': int(listing.odoo_qty),
                })
                price_items.append({
                    'barcode': listing.barcode,
        ...
        return {
            ...
            'params': {
                'message': _(
                    "%d ürün için stok/fiyat PttAVM'e gönderildi."
                ) % len(self),
                'type': 'success',
                'sticky': False,
            },
