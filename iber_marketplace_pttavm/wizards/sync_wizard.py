...
class IberPttavmSyncWizard(models.TransientModel):
    _name = 'iber.marketplace.pttavm.sync.wizard'
    _description = 'PttAVM Senkronizasyon Sihirbazı'

    config_id = fields.Many2one(
        'iber.marketplace.pttavm.config', required=True, string='Bağlantı')
    ...
    order_status_filter = fields.Char(
        ...
        help='Örn: WaitingForApproval — tek bir PttAVM durum değeri')
    ...
    def _sync_orders(self):
        ...
        # PttAVM ISO8601 format kullanır
        ...
        page = 1
        page_size = 100
        Order = self.env['iber.marketplace.pttavm.order']
        ...
        while True:
            try:
                ...
                raise UserError(_('PttAVM sipariş çekme hatası: %s') % exc)

            # PttAVM yanıt yapısı: {'data': [...]} veya doğrudan liste
            ...
            for raw in content:
                pttavm_order_id = str(raw.get('id') or raw.get('orderId') or '')
                if not pttavm_order_id:
                    continue

                existing = Order.search([
                    ('config_id', '=', self.config_id.id),
                    ('pttavm_order_id', '=', pttavm_order_id),
                ], limit=1)
                ...
                if existing:
                    ...
                    for ld in line_data:
                        ...
                        self.env['iber.marketplace.pttavm.order.line'].create(ld[2])
                    updated += 1
                else:
    ...
    def _sync_inventory(self):
        ...
        self.result_message = _(
            "%d ürün için stok/fiyat PttAVM'e gönderildi.") % len(listings)
    ...
    def _map_order_vals(self, o):
        """PttAVM sipariş dict'ini Odoo alan değerlerine çevirir."""
        # Müşteri adı
        ...
        return {
            'pttavm_order_id': str(o.get('id') or o.get('orderId') or ''),
            ...
            'cargo_tracking_no': o.get('cargoTrackingNo') or '',
            'cargo_tracking_url': o.get('cargoTrackingUrl') or '',
            'line_ids': lines,
        }

    def _map_line_vals(self, line):
        """PttAVM sipariş satır dict'ini Odoo alan değerlerine çevirir."""
        return {
            'pttavm_line_id': str(line.get('id') or ''),
